    ...
    Papa.parse(fileData, {
        complete: function(results) {
            ...
            for (let i = 1; i < parsedData.length; i++) {
                ...
                if (columnIndexNetWeight !== -1 && row[columnIndexNetWeight] && !isNaN(row[columnIndexNetWeight])) {
                    totalNetWeight += parseFloat(row[columnIndexNetWeight]);
                }
                if (columnIndexGrossWeight !== -1 && row[columnIndexGrossWeight] && !isNaN(row[columnIndexGrossWeight])) {
                    totalGrossWeight += parseFloat(row[columnIndexGrossWeight]);
                }
                if (columnIndexQuantity !== -1 && row[columnIndexQuantity] && !isNaN(row[columnIndexQuantity])) {
                    totalQuantity += parseFloat(row[columnIndexQuantity]);
                }
                if (columnIndexCartons !== -1 && row[columnIndexCartons] && !isNaN(row[columnIndexCartons])) {
