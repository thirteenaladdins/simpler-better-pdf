...
# Okay, now lets test chat GPT and see how it optimise my code

def extract_luxury_goods_data_2(file):
    item_list = []
    full_text = Helpers.convert_all_pages_to_text(file)
    invoice_no = extract_invoice_no(file)
    ...
    total_net_weight, total_gross_weight, total_cartons = (extract_net_weight(full_text), 
    extract_gross_weight(full_text), extract_total_packages(full_text))
    
    formatted_items = format_items(all_matches)
    
    formatted_items = [item + [invoice_no] for item in formatted_items]

    # using list comprehension and zip to generate quantity and add descriptions
    item_list = [i + [j] for i, j in zip(match_descriptions(formatted_items, descriptions_list), find_quantity_in_list(all_matches, 1))]

    # Create dataframe from item_list and add Total Gross Weight, Total Net Weight and Total Cartons
    df = pd.DataFrame(item_list, columns = ["Commodity Code", "Value", "Country of Origin", "Invoice", "Description", "Quantity"])
    df.loc[0,'Total Gross Weight'] = total_gross_weight
    df.loc[0,'Total Net Weight'] = total_net_weight
    df.loc[0,'Total Cartons'] = total_cartons
    df = df.sort_values(by=['Commodity Code'])
    df[['Commodity Code', 'Quantity', 'Value']] = df[['Commodity Code', 'Quantity', 'Value']].apply(pd.to_numeric, errors='coerce')
    manually_extracted_totals = df.groupby(['Commodity Code'], as_index=False).sum()
    manually_extracted_totals = manually_extracted_totals[['Commodity Code', 'Quantity', 'Value']]
    totals = extract_total_table_data(full_text)
    totals = totals.drop(columns=['Description', 'Net Weight'])
    totals[['Commodity Code', 'Quantity', 'Value']] = totals[['Commodity Code', 'Quantity', 'Value']].apply(pd.to_numeric, errors='coerce')
    manually_extracted_totals['quantities_match'] = np.where(manually_extracted_totals['Quantity'] == totals['Quantity'], 'True', 'False')
    manually_extracted_totals['value_match'] = np.where(manually_extracted_totals['Value'] == totals['Value'], 'True', 'False')
    print(manually_extracted_totals)
