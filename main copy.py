...
def extract_items(full_text):
    all_matches = re.findall(r'Item.*?Made in.*?[A-Z]\n', full_text, re.DOTALL)
    # all_matches = re.findall(r'Item.*?Made in', full_text, re.DOTALL)
    # get the item after made in which is the COO 
    # so it goes all the way to made
    print(all_matches)
...
def calculate_quantity(item_list, value):
    ...
    # we're calculating quantity through some crazy method
    final_list = []
    ...
    for item in item_list:
        ...
        print(item_list_to_process, 'PROCESS_ITEM')
        for value in item_list_to_process:
            if ',' in value:
                # if there are no alpha characters
                if not re.search(alpha, value):
                    find_value.append(float(value.replace('.', "").replace(',', '.')))

        
        #### EDGE CASE ####
        # TODO perhaps we first divide the two values - 
        # smallest by largest - if the value is within a certain threshold 
        # we decide whether or not to take the smaller or bigger value
        # I want the total value of the items but if there's a single item
        # 1 item value with a discount - doesn't work for that case
        # we need new logic to figure out which of the values is the correct one
        # if there is no discount it work because there will only be one value in the list
        # as we remove all duplicates

        # discount values are harder
        print(find_value, 'FIND_VALUE')
        # value appears twice - both with 

        value = max(find_value)
        # value = item_list_to_process[index - 2].replace('.', "").replace(',', '.')
        
        # if preceed by a zero it is not a value we need to deal with
            ...
            if not re.search(alpha, new_item):
                if re.search(num, new_item):
                    ...
                    # remove_duplicates_list = set(j)
                        ...
                        # REMOVE ANY VALUES WITH A LEADING ZERO
                        # [i for i in remove_duplicates]
                        remove_leading_zeroes = [i for i in remove_duplicates if i[0] is not '0']


        # print(remove_leading_zeroes, "REMOVE DUPLICATES")                        
...
def format_items(all_items):
    ...
    for item in all_items:
        ...
        # here we check the min and max values
